...
proc autosetup_install {} {
	if {![file exists auto.def] && ![opt-bool force]} {
		user-error "No auto.def found in the current directory. Use --force for a new project."
	}
	...
	if {[catch {
		...
		foreach file {config.guess config.sub jimsh0.c find-tclsh} {
			autosetup_install_file $::autosetup(dir)/$file autosetup
		}
		exec chmod 755 autosetup/config.* autosetup/find-tclsh
